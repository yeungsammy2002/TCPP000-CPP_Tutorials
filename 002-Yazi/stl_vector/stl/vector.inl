...
typename Vector<T>::Iterator Vector<T>::erase(const Iterator & it)
{
    ...
    if (is_basic_type())
    {
        std::memmove(m_data + size, m_data + size + 1, sizeof(T) * (m_size - size));
    } else
    {
        for (int i = size; i < m_size; ++i)
        {
...
typename Vector<T>::Iterator Vector<T>::erase(const Iterator & first, const Iterator & last)
{
    ...
    if (is_basic_type())
    {
        std::memmove(m_data + f, m_data + l, sizeof(T) * (m_size - 1));
    } else
    {
...
{
//    std::cout << "is basic type" << std::endl;
