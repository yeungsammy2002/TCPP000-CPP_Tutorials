...
Socket::Socket() : m_ip(""), m_port(0), m_sockfd(0)
{
    ...
    if (0 > m_sockfd)
        std::printf("create socket error: errno=%d errmsg=%s\n", errno, std::strerror(errno));
    else
        std::printf("create socket success!\n");
}
...
std::size_t Socket::recv(char * buff, std::size_t len)
{
    return ::recv(m_sockfd, buff, len, 0);
}

std::size_t Socket::send(const char * buff, std::size_t len)
{
...
{
    int buff_sz = size;
    if (0 > setsockopt(m_sockfd, SOL_SOCKET, SO_SNDBUF, &buff_sz, sizeof(buff_sz)))
    {
...
{
    int buff_sz = size;
    if (0 > setsockopt(m_sockfd, SOL_SOCKET, SO_RCVBUF, &buff_sz, sizeof(buff_sz)))
    {
...
bool Socket::set_keepalive()
{
    ...
    {
        std::printf("socket set_keepalive() error: errno=%d errmsg=%s\n", errno, std::strerror(errno));
        return false;
    }
    return true;
}
